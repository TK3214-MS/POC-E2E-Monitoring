...
for (let index = 0; index < 30; index++) { 
    test('mypage_03_' + index, async ({ page }) => {
        ...
        await page.frameLocator('iframe[title="ポロシャツ \\[Dev\\]"]').getByRole('button', { name: '__カートに追加して別のデザインを作成' }).click();

        // Set url go to screen Cart
        await page.goto('https://harada-front-dev.azurewebsites.net/?page_id=cart');
        await page.getByRole('button', { name: '__デザイン確認を依頼する' }).click();
        await page.getByRole('button', { name: '__トップページへ' }).click();
    });
}
